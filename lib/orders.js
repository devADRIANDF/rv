...
async function registrarComisionSiAplica(orderId, email, refCode, amountCents) {
  ...
  await dbRun(
    "INSERT INTO referral_earnings (affiliate_id, from_user_id, from_email, order_id, amount_cents) VALUES (?, NULL, ?, ?, ?)",
    [afiliado.id, email, orderId, comision]
  );
  await dbRun("UPDATE users SET wallet_balance = wallet_balance + ? WHERE id = ?", [comision, afiliado.id]);
}
